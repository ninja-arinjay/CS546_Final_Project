    ...
    try {
      ...
      if (!memberLimit) {
        ...
        if (typeof memberLimit !== "number")
          throw "Error : Member Limit must be a number.";
        if (memberLimit <= 0)
          throw "Error : Member Limit can not be negative or zero.";
        if (!Number.isInteger(memberLimit))
          throw "Error : Member Limit should be a whole number.";
      }
      if (!ageMin) {
        ageMin = -1;
      } else {
        if (typeof ageMin !== "number") throw "Error : Age must be a number.";
        //We can keep a lower bound if we wish to
        if (ageMin <= 16)
          throw "Error : Minimum Age can not be negative or zero.";
        if (!Number.isInteger(ageMin))
          throw "Error : Minimum Age should be a whole number.";
      }
      let comments = [];
